...
# Εδώ αποθηκεύουμε την τρέχουσα (ενεργή) καμπάνια που αποδέχτηκε ο παίκτης.
if "active_campaign" not in st.session_state:
    st.session_state["active_campaign"] = None  # ή λεξικό με info
...
# =============== TAB 1: BOARD GAME ===============
with tabs[0]:
    st.title("Maritime Board Game (with Sponsor Campaign)")
    ...
    # ----------- Sponsor Campaign Offer (Now just sets active_campaign if accepted) -----------
    def maybe_offer_sponsored_campaign():
        ...
        if chance < 0.33:
            ...
            st.info(f"**Sponsored Campaign Offer**: {sponsor} ζητάει {required_impressions} impressions για έκπτωση {discount_percent}%. Δέχεσαι;")
            ...
            if accept:
                ...
                beaches_list = ["Lindos", "Faliraki", "Tsambika"]
                st.session_state["active_campaign"] = {
                    ...
                    "materials": f"Banners and T-shirts from {sponsor}"
                }
                st.success(f"Πήρες μέρος στην καμπάνια της {sponsor}. Δες λεπτομέρειες στην καρτέλα 'Campaign Details'.")
                
            elif decline:
                st.warning("Απέρριψες την καμπάνια.")
    ...
    if st.button("Roll the Dice"):
        if st.session_state["skip_turn"]:
            ...
            # Προσπάθεια για τυχαία προσφορά
            if not st.session_state["already_offered_campaign"]:
                chance = random.random()
                if chance < 0.33:
                    # Δείχνουμε μια προσφορά
                    sponsor_names = ["Vodafone", "Nike", "Adidas", "Coca-Cola", "Pepsi"]
                    sponsor = random.choice(sponsor_names)
                    required_impressions = random.randint(500, 2000)
                    discount_percent = random.choice([20, 30, 50, 70])
                    ...
                    if accept:
                        ...
                        st.session_state["active_campaign"] = {
                            ...
                            "end_date": endD,
                            "beaches": beaches_list,
                            "materials": f"Banners & T-shirts from {sponsor}"
                        }
                        st.success(f"Αποδέχτηκες την καμπάνια της {sponsor}! Δες λεπτομέρειες στην καρτέλα 'Campaign Details'.")
                    elif decline:
                        st.warning("Απέρριψες την καμπάνια.")
