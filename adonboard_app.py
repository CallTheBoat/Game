...
st.title("Interactive Maritime Board Game with Ad Campaigns")
...
# ---------- Board Game Setup ----------
# Ορισμός 10 board squares με διάφορα events
BOX_SIZE = 0.03  
...
if st.button("Roll the Dice"):
    if st.session_state["skip_turn"]:
        ...
        # If Ad Zone event, display options
                ...
                if st.button("Join Ad Campaign", key=f"join_ad_{new_index}"):
                    ...
                    with st.form(key=f"ad_campaign_form_{new_index}"):
                        ...
                        if submit_campaign:
                            ...
                            st.success(f"Campaign joined! +{bonus_campaign} ad score, +{campaign_impressions} impressions.")
                            # Calculate campaign dates
                            start_date = date.today()
                            end_date = start_date + timedelta(days=duration_days)
                            st.markdown(f"**Campaign Dates:** {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
                            # Display placeholder image and beaches
